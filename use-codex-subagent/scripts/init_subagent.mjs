...
const BASE_MODEL_PROVIDERS = [
	{ key: "gpt-5.6-luna", name: "OpenAI Base", model: "gpt-5.6-luna" },
	{ key: "gpt-5.6-sol", name: "OpenAI Base", model: "gpt-5.6-sol" },
...
function parseConfig(configPath) {
	...
	for (const m of text.matchAll(providerRe)) {
		...
		if (isCustom) {
			...
			apiKey =
				extractField(block, "api_key") ??
				extractField(block, "experimental_bearer_token");
		}
...
async function enrichCustomProviders(providers) {
	...
	for (const p of providers) {
		...
		if (models.length === 0) {
			console.error(
				`  ⚠ ${p.name} 模型列表为空或不可用，回退到配置中的模型 "${p.model || "(无)"}"`,
			);
			result.push(p);
			continue;
		}
...
function printSummary(selections) {
	...
	for (const s of selections) {
		const key = s.key ?? "(未匹配)";
		console.log(
			`  ${s.role.padEnd(12)}${key.padEnd(28)}${s.model.padEnd(22)}${s.thinking}`,
		);
	}
}
...
function renderAgentToml(roleSpec, model, thinking) {
	const nicknames = roleSpec.nicknameCandidates
		.map((n) => `"${tomlEscape(n)}"`)
		.join(", ");
...
async function interactiveSetup(providers) {
	...
	for (const role of ROLES) {
		...
		console.log(
			`   默认: #${defaultLabel} (${role.defaultModel})  |  思考: ${role.defaultThinking}\n`,
		);

		let provider;
		while (true) {
			const ans = (
				await ask(`   选择模型 (编号/Key) [#${defaultLabel}]: `)
			).trim();
			if (!ans) {
		...
		let thinking;
		while (true) {
			const ans = (await ask(`   思考强度 [${role.defaultThinking}]: `))
				.trim()
				.toLowerCase();
			if (!ans) {
				thinking = role.defaultThinking;
				break;
			}
			if (/^\d+$/.test(ans)) {
				const idx = +ans - 1;
				if (idx >= 0 && idx < THINKING_LEVELS.length) {
					thinking = THINKING_LEVELS[idx];
					break;
				}
...
const options = {
	...
	"target-dir": {
		type: "string",
		default: resolve(homedir(), ".codex/agents"),
	},
...
const selections = ROLES.map((role) => {
	...
	if (modelInput) {
		matched =
			providers.find((p) => p.key === modelInput) ??
			providers.find((p) => p.model === modelInput);
	}
	if (!matched) {
...
if (args.json) {
	const config = Object.fromEntries(
		selections.map((s) => [
			s.role,
			{ key: s.key, model: s.model, thinking: s.thinking },
		]),
	);
